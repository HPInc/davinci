...
import { checkAndAssignBootDir, execBootScripts } from '../../../../src/express/boot';

describe('boot', () => {
	let cwdStub;

	before(() => {
		// set our current working directory to the stub application
		cwdStub = sinon.stub(process, 'cwd').callsFake(() => `${__dirname}/stubApp/`);
	});
